...
const mockCommunities = [
  ...
  { id: 2, name: "مشاريع التخرج", type: "خاص", category: "قسم المشاريع" },
];
...
const CommunityPage = () => {
  ...
  const [newCommunity, setNewCommunity] = useState({ name: "", type: "عام", password: "" });
  ...
  const handleCreateCommunity = () => {
    ...
    const newCom = { ...newCommunity, id: Date.now(), category: "قسم جديد" };
    setCommunities([...communities, newCom]);
    setNewCommunity({ name: "", type: "عام", password: "" });
    setShowCreate(false);
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-orange-50 min-h-screen community-page">
      <h1 className="text-3xl font-bold text-orange-600 mb-4 community-title">مجتمع الأساتذة</h1>

      <div className="flex gap-4 items-center community-filter">
        <select
      ...
      {showCreate && (
        <div className="mt-6 space-y-4 create-form">
          <h2 className="text-xl font-semibold text-orange-600">إنشاء مجتمع جديد</h2>
          <input
            className="styled-input"
          ...
          <button onClick={handleCreateCommunity} className="send-button">
            إنشاء
          </button>
        </div>
      )}
