...
export default function CoursePage() {
  ...
  const toggleComplete = (path) => {
    const updateCompletion = (nodes, currentPath = []) => {
      return nodes.map((node, idx) => {
        ...
        if (node.children) {
          return {
            ...node,
            children: updateCompletion(node.children, fullPath),
          };
        }
        return node;
      });
    };
    setCourses(updateCompletion(courses));
  };
...
const renderTree = (nodes, path = []) => (
        ...
        <li key={index} style={{ position: 'relative', paddingLeft: '2rem', marginBottom: '2rem' }}>
          ...
          >
            ...
            <input
              ...
              onChange={() => toggleComplete(currentPath)}
            />
          </div>
      ...
      <main style={{ width: '80%', padding: '2rem' }}>
        ...
        <div style={{ marginTop: '1rem', display: 'flex', gap: '1rem' }}>
          <button
            ...
            style={{ padding: '0.5rem 1rem', backgroundColor: activeTab === "courses" ? '#FFCC99' : '#FFDAB9', border: 'none', borderRadius: '5px' }}
          >
            المواد
          </button>
          <button
            onClick={() => setActiveTab("progress")}
            style={{ padding: '0.5rem 1rem', backgroundColor: activeTab === "progress" ? '#FFCC99' : '#FFDAB9', border: 'none', borderRadius: '5px' }}
          >
            التقدم
          </button>
        </div>
  ...
  <div style={{ marginTop: '1.5rem' }}>
    ...
    {/* عرض المواد غير المكتملة بشكل شبيه بالشجرة */}
